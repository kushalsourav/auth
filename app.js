...
app.use(checkToken, (req, res, next) => {
  dataBase.User.findById(req.token.id).then((user) => {
    if (!user) {
...
app.post("/users/habit/update/:habitId", checkToken, async (req, res) => {
  ...
  try {
    await dataBase.User.findByIdAndUpdate(req.token.id).then((user) => {
      ...
      const habitIndex = user.habits.findIndex((habit) =>
        habit._id.equals(habitId),
      );
...
app.post("/user/goals/", checkToken, (req, res) => {
  const { name, dateAdded, icon, colorCode } = req.body;
  ...
  dataBase.User.findByIdAndUpdate(
    ...
    if (!user) {
      ...
      res.json({ success: true, result: "Goal completed", user: user });
    }
  });
});
...
//token eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY1MTAxM2E3ZGFjYmEwNWRhYTRkNDZhYyIsImVtYWlsIjoia3VzaGFsc291cmF2MkBnbWFpbC5jb20iLCJpYXQiOjE2OTU4OTE2MTd9.fQ2zo_aQACSFevocDYFZS30VWBOvdLQy2jHQAHWnPkk
